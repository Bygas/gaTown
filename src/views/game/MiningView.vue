  ...
  <div>
    <div class="flex items-center justify-between mb-3">
      <h3 class="text-accent text-sm">
        ...
        {{ miningStore.isInSkullCavern ? 'Kafatası Mağarası' : 'Bulutgölgesi Madeni' }}
      </h3>
    ...
    <div v-if="miningStore.isSkullCavernUnlocked()" class="border border-accent/20 rounded-xs p-3 mb-4">
      <div class="flex items-center justify-between mb-1">
        ...
        <span v-else class="text-xs text-muted/40">Henüz keşfedilmedi</span>
      </div>
      <p class="text-xs text-muted">Sonsuz kat · Güvenli nokta yok · İridyum kaynağı · Canavarlar derine indikçe güçlenir</p>
    </div>
    ...
    <div class="border border-accent/20 rounded-xs p-3 mb-4">
      ...
      <div class="flex flex-col space-y-1">
        ...
        <div class="flex items-center justify-between border border-accent/10 rounded-xs px-3 py-1.5">
          <span class="text-xs">Saldırı Gücü</span>
          <span class="text-xs text-accent">{{ weaponAttack }}</span>
        </div>
        <div class="flex items-center justify-between border border-accent/10 rounded-xs px-3 py-1.5">
          <span class="text-xs">Tür · Kritik</span>
        ...
        <div class="flex items-center justify-between border border-accent/10 rounded-xs px-3 py-1.5">
          <span class="text-xs">Enerji</span>
    ...
    >
      <div class="flex items-center space-x-1.5">
        ...
        <span class="text-sm text-accent">Keşfe Çık</span>
      </div>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full">
          ...
          <p class="text-xs text-muted mb-2">Güvenli nokta: {{ miningStore.safePointFloor > 0 ? `${miningStore.safePointFloor}. kat` : 'Giriş' }}</p>
            ...
            >
              <div class="flex justify-between items-center text-xs mb-1">
                <span :class="zone.isCurrentZone ? 'text-accent' : zone.reached ? 'text-text' : 'text-muted/40'">
                  {{ zone.name }}
                  <span class="text-muted ml-1">{{ zone.start }}-{{ zone.end }}. katlar</span>
                </span>
        ...
        <div class="game-panel max-w-xs w-full relative">
          ...
          >
            <span class="text-xs text-danger">
              ...
              Kafatası Mağarası'na Gir
            </span>
            <span class="text-xs text-muted">Sonsuz kat</span>
          </div>
        </div>
      </div>
    </Transition>
        ...
        <div class="game-panel max-w-sm w-full">
          ...
          <div class="flex items-center justify-between mb-2">
            <p class="text-sm text-accent">
              ...
              <span v-if="currentFloorSpecial === 'mushroom'" class="text-success ml-1">Mantar Mağarası</span>
              <span v-if="currentFloorSpecial === 'treasure'" class="text-accent ml-1">Hazine Katı</span>
              <span v-if="currentFloorSpecial === 'infested'" class="text-danger ml-1">İstilalı Kat</span>
              <span v-if="currentFloorSpecial === 'dark'" class="text-muted ml-1">Karanlık Nehir Katı</span>
              <span v-if="currentFloorSpecial === 'boss'" class="text-danger ml-1">Patron Katı</span>
            </p>
          ...
          <div class="text-xs text-muted mb-2 border-b border-accent/20 pb-2 space-y-0.5">
            <p>
              ...
              {{ weaponDisplayName }}（{{ weaponTypeName }} · Saldırı {{ weaponAttack }} · Kritik {{ critRateDisplay }}）
            </p>
            <p v-if="weaponEnchantName" class="text-success">Büyü: {{ weaponEnchantName }}</p>
          </div>

          <!-- İstilalı kat uyarısı -->
          <p v-if="currentFloorSpecial === 'infested' && remainingMonsters > 0" class="text-xs text-danger mb-2">
            İstilalı kat: Daha {{ remainingMonsters }} canavar yenmen gerekiyor
          </p>

          <!-- Bomba modu -->
          <div v-if="bombModeId" class="text-xs text-accent mb-2 border border-accent/30 rounded-xs px-2 py-1">
            <Zap :size="12" class="inline" />
            Bomba modu: Patlama merkezini seçmek için keşfedilmiş bir kareye dokun
            <button class="text-muted ml-2 underline" @click="bombModeId = null">İptal</button>
          </div>
          ...
          <div class="flex flex-col space-y-1 mb-3">
            ...
            >
              ...
              <span v-if="!miningStore.stairsUsable" class="text-xs text-muted">Merdiven kullanılamıyor</span>
            </div>
            <div
              class="flex items-center justify-between border border-danger/30 rounded-xs px-3 py-1.5 cursor-pointer hover:bg-danger/5"
            ...
            >
              <span class="text-xs text-danger">
                ...
                {{ miningStore.isInSkullCavern ? 'Kafatası Mağarası'ndan Ayrıl' : 'Madenden Ayrıl' }}
              </span>
            </div>
          </div>
        ...
        <div class="game-panel max-w-xs w-full">
          ...
          <div class="flex items-center justify-between mb-2">
            <p class="text-sm" :class="miningStore.combatIsBoss ? 'text-danger' : 'text-accent'">
              {{ miningStore.combatIsBoss ? 'Patron Savaşı' : 'Canavarla Karşılaştın' }}
            </p>
          </div>
          ...
          <div class="grid grid-cols-[1fr_auto_1fr] gap-1.5 mb-3 items-center">
            ...
            <div class="border border-danger/20 rounded-xs p-2 relative" :class="monsterAnim">
              <p class="text-xs text-center text-danger mb-1.5 truncate">
                ...
                <span v-if="miningStore.combatIsBoss" class="text-[10px]">[PATRON]</span>
              </p>
              <div class="bg-bg rounded-xs h-1.5 mb-1">
                <div
          ...
          <div class="mb-3 space-y-1">
            ...
            <div class="grid grid-cols-3 gap-1">
              ...
              >
                ...
                <span class="text-[10px] text-muted">{{ weaponAttack }} saldırı</span>
              </div>
              <div
                class="flex flex-col items-center border border-accent/20 rounded-xs py-1.5"
                :class="combatAnimLock ? 'opacity-50' : 'cursor-pointer hover:bg-accent/5'"
              ...
              >
                ...
                <span class="text-[10px] text-muted">Hasarı azalt</span>
              </div>
              <div
              ...
              >
                <span class="text-xs" :class="miningStore.combatIsBoss ? 'text-muted' : 'text-danger'">
                  ...
                  {{ miningStore.combatIsBoss ? 'Kaçılamaz' : 'Kaç' }}
                </span>
                <span v-if="miningStore.combatIsBoss" class="text-[10px] text-muted/40">Patron savaşı</span>
              </div>
            </div>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full">
          ...
          <p v-if="availableCombatItems.length === 0" class="text-xs text-muted text-center py-2">Kullanılabilir eşya yok</p>
        </div>
      </div>
    </Transition>
        ...
        <div class="game-panel max-w-xs w-full relative">
          ...
          <div class="border border-accent/10 rounded-xs p-2 mb-2">
            ...
            <div class="flex items-center justify-between mt-0.5">
              <span class="text-xs text-muted">Etki</span>
          ...
          <div class="flex space-x-1.5">
            <Button class="flex-1 justify-center" @click="pendingItemId = null">İptal</Button>
    ...
    <Transition name="panel-fade">
      ...
      >
        <div class="game-panel max-w-xs w-full">
          <p class="text-sm text-accent mb-2">Ayrılmayı Onayla</p>
          <p class="text-xs text-muted mb-3">
            {{ miningStore.isInSkullCavern ? 'Kafatası Mağarası'ndan' : 'Madenden' }} ayrılmak istediğine emin misin? Mevcut ilerleme korunmaz.
          </p>
          <div class="flex space-x-1.5">
            <Button class="flex-1 justify-center" @click="showLeaveConfirm = false">Keşfe Devam Et</Button>
            <Button class="flex-1 justify-center btn-danger" :icon="LogOut" @click="confirmLeave">Ayrıl</Button>
          </div>
        </div>
      </div>
    </Transition>

    <!-- Hızlı ekipman: plan listesi -->
    <Transition name="panel-fade">
      <div
      ...
      >
        <div class="game-panel max-w-xs w-full relative">
          ...
          <div v-if="inventoryStore.equipmentPresets.length > 0" class="flex flex-col space-y-1.5 max-h-60 overflow-y-auto">
            ...
            >
              <div class="flex items-center justify-between mb-1">
                ...
                <span v-if="inventoryStore.activePresetId === preset.id" class="text-[10px] text-success shrink-0 ml-1">Kullanımda</span>
              </div>
              <div class="grid grid-cols-2 gap-1">
                ...
                <Button class="py-0 px-1.5 text-[10px]" @click="viewPresetDetail(preset.id)">İncele</Button>
              </div>
            </div>
          </div>
          <div v-else class="flex flex-col items-center justify-center py-6">
            ...
            <p class="text-[10px] text-muted/60 mt-0.5">Bir plan oluşturmak için çantandaki ekipman sayfasına git</p>
          </div>
        </div>
      </div>
    </Transition>

    <!-- Hızlı ekipman: plan detayı -->
    <Transition name="panel-fade">
      <div
          ...
          <div class="flex flex-col space-y-1">
            ...
            >
              ...
              <span class="text-xs" :class="detailPreset.weaponDefId ? 'text-accent' : 'text-muted/40'">
                {{ detailPreset.weaponDefId ? (getWeaponById(detailPreset.weaponDefId)?.name ?? 'Bilinmiyor') : 'Yok' }}
              </span>
            </div>
            <div
              class="flex items-center justify-between border border-accent/10 rounded-xs px-3 py-1.5"
            ...
            >
              ...
              <span class="text-xs" :class="detailPreset.ringSlot1DefId ? 'text-accent' : 'text-muted/40'">
                {{ detailPreset.ringSlot1DefId ? (getRingById(detailPreset.ringSlot1DefId)?.name ?? 'Bilinmiyor') : 'Yok' }}
              </span>
            </div>
            <div
              class="flex items-center justify-between border border-accent/10 rounded-xs px-3 py-1.5"
            ...
            >
              ...
              <span class="text-xs" :class="detailPreset.ringSlot2DefId ? 'text-accent' : 'text-muted/40'">
                {{ detailPreset.ringSlot2DefId ? (getRingById(detailPreset.ringSlot2DefId)?.name ?? 'Bilinmiyor') : 'Yok' }}
              </span>
            </div>
            <div
              class="flex items-center justify-between border border-accent/10 rounded-xs px-3 py-1.5"
            ...
            >
              <span class="text-xs text-muted">Şapka</span>
              <span class="text-xs" :class="detailPreset.hatDefId ? 'text-accent' : 'text-muted/40'">
                {{ detailPreset.hatDefId ? (getHatById(detailPreset.hatDefId)?.name ?? 'Bilinmiyor') : 'Yok' }}
              </span>
            </div>
            <div
              class="flex items-center justify-between border border-accent/10 rounded-xs px-3 py-1.5"
            ...
            >
              <span class="text-xs text-muted">Ayakkabı</span>
              <span class="text-xs" :class="detailPreset.shoeDefId ? 'text-accent' : 'text-muted/40'">
                {{ detailPreset.shoeDefId ? (getShoeById(detailPreset.shoeDefId)?.name ?? 'Bilinmiyor') : 'Yok' }}
              </span>
            </div>
          </div>
        </div>
    ...
    <!-- Hızlı ekipman: eşya özellikleri -->
    <Transition name="panel-fade">
      <div
...
<script setup lang="ts">
  ...
  const tutorialHint = computed(() => {
    ...
    if (achievementStore.stats.highestMineFloor === 0)
      return 'Maden 6x6 karelerden oluşur. Karelere dokunarak keşif yaparsın. Cevher bulursan kazabilirsin, canavar çıkarsa savaşman gerekir. Merdiveni bulursan alt kata inebilirsin.'
    return null
  })
  ...
  /** Savaşta kullanılabilir eşyalar */
  const availableCombatItems = computed(() => {
    ...
    if (badgeCount > 0) {
      items.push({ itemId: 'guild_badge', name: 'Lonca Rozeti', desc: 'Saldırı kalıcı olarak +3', count: badgeCount })
    }

    const talismanCount = inventoryStore.getItemCount('life_talisman')
    if (talismanCount > 0) {
      items.push({ itemId: 'life_talisman', name: 'Yaşam Tılsımı', desc: 'Maksimum can kalıcı olarak +15', count: talismanCount })
    }

    const coinCount = inventoryStore.getItemCount('lucky_coin')
    if (coinCount > 0) {
      items.push({ itemId: 'lucky_coin', name: 'Şans Parası', desc: 'Düşme oranı kalıcı olarak +5%', count: coinCount })
    }

    const defenseCharmCount = inventoryStore.getItemCount('defense_charm')
    if (defenseCharmCount > 0) {
      items.push({ itemId: 'defense_charm', name: 'Koruma Tılsımı', desc: 'Savunma kalıcı olarak +3%', count: defenseCharmCount })
    }

    if (!miningStore.slayerCharmActive) {
      const charmCount = inventoryStore.getItemCount('slayer_charm')
      if (charmCount > 0) {
        items.push({ itemId: 'slayer_charm', name: 'Avcı Tılsımı', desc: 'Bu keşifte düşme oranı +20%', count: charmCount })
      }
    }
    ...
    for (const invItem of inventoryStore.items) {
      ...
      const parts: string[] = []
      if (def.healthRestore) parts.push(def.healthRestore >= 999 ? 'Canı tamamen doldurur' : `Can +${def.healthRestore}`)
      if (def.staminaRestore) parts.push(`Enerji +${def.staminaRestore}`)
  ...
  /** Maden haritası bölge verileri */
  const mineZones = computed(() => {
    const zones = [
      { id: 'shallow', name: 'Sığ Maden · Taşlık Mağara', start: 1, end: 20, bossFloor: 20 },
      { id: 'frost', name: 'Buz Mağarası · Donmuş Yeraltı Nehri', start: 21, end: 40, bossFloor: 40 },
      { id: 'lava', name: 'Lav Katı · Yeraltı Ateşi', start: 41, end: 60, bossFloor: 60 },
      { id: 'crystal', name: 'Kristal Mağarası · Kristal Labirent', start: 61, end: 80, bossFloor: 80 },
      { id: 'shadow', name: 'Gölgeler Diyarı · Karanlık Yarık', start: 81, end: 100, bossFloor: 100 },
      { id: 'abyss', name: 'Uçurum · Sonsuz Derinlik', start: 101, end: 120, bossFloor: 120 }
    ]
  ...
  const weaponTypeName = computed(() => {
    ...
    return def ? WEAPON_TYPE_NAMES[def.type] : 'Bilinmiyor'
  })
  ...
  /** Asansör katlarını bölgelere ayır */
  const elevatorZones = computed(() => {
    ...
    const zones = [
      { name: 'Sığ Maden', min: 0, max: 20 },
      { name: 'Buz Mağarası', min: 21, max: 40 },
      { name: 'Lav Katı', min: 41, max: 60 },
      { name: 'Kristal Mağarası', min: 61, max: 80 },
      { name: 'Gölge Diyarı', min: 81, max: 100 },
      { name: 'Uçurum', min: 101, max: 120 }
    ]
    return zones
      .map(z => ({
        name: z.name,
  ...
  const handleTileClick = (tile: MineTile) => {
    if (gameStore.isPastBedtime) {
      addLog('Artık çok geç, keşfe devam edemezsin.')
      handleEndDay()
      return
    }
  ...
  const handleCombat = (action: CombatAction) => {
    ...
    if (action === 'attack' && dealt > 0) {
      ...
      showDamageFloat('monster', isCrit ? `Kritik -${dealt}` : `-${dealt}`)
    }
    if (action === 'defend') {
  ...
  const handleNextFloor = () => {
    if (gameStore.isPastBedtime) {
      addLog('Artık çok geç, geri dönme vakti geldi.')
      handleEndDay()
      return
    }
    showCombatItems.value = false
  ...
  const EFFECT_NAMES: Record<EquipmentEffectType, string> = {
    ...
    treasure_find: 'Hazine Bulma Şansı',
  ...
  const viewEquipProperty = (type: 'weapon' | 'ring' | 'hat' | 'shoe', defId: string) => {
    if (type === 'weapon') {
      ...
      equipPropertyInfo.value = {
        ...
        effects: [
          { label: 'Saldırı Gücü', value: `${def.attack}` },
          { label: 'Tür', value: WEAPON_TYPE_NAMES[def.type] },
          { label: 'Kritik Oranı', value: `${Math.round(def.critRate * 100)}%` }
        ]
      }
    } else if (type === 'ring') {
